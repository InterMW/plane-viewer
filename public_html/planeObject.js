...
function PlaneObject(icao) {
    ...
    //state
    this.is_history_requesting = false;
    ...
    if (this.track_linesegs.length == 0) {
        ...
        fetch(url+ this.icao)
            .then(function(response) {
                ...
                try {
                    ...
                    do {
                        ...
                        if (response.ok) {
                            ...
                            pointer = jsonResponse.previousLink;
                        }
                        else
                        {
                            done = true;
                ...
                var newseg = {
                    track: new google.maps.MVCArray(results),
                    line: null,
                    head_update: plane.last_position_time,
                    tail_update: plane.last_position_time,
                    estimated: false,
                    ground: (plane.altitude === "ground")
                };
                plane.track_linesegs.push(newseg);
                plane.history_size += results.length;
